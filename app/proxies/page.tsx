...
import { Column, getReverseProxies, getStreams, ReverseProxy, ReverseProxyColumns, Stream, StreamColumns } from "@/types/proxy";
...
function reloadButton(list: AsyncListData<Stream | ReverseProxy>) {
  return (
    <Button isIconOnly onPress={list.reload} variant="light">
...
export default function ProxiesPage() {
  ...
  function table(key_prefix: string, columns: Column[], list: AsyncListData<Stream | ReverseProxy>) {
    return (
      <Table
      ...
      >
        <TableHeader columns={columns} >
          {(col) => <TableColumn key={col.key} allowsSorting>{col.label}</TableColumn>}
        </TableHeader>
        <TableBody
          items={list.items}
  ...
  return (
    <div>
      <Tabs
        aria-label="tabs"
        variant="light"
        onSelectionChange={(key) => setActiveList(key === "reverse_proxies" ? rps : streams)}
      >
        ...
        <Tab key="reload" title={reloadButton(activeList)}> </Tab>
      </Tabs>
    </div>
  );
}
