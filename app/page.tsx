          ...
          <CardBody className="py-5">
            ...
            <div className="flex gap-6 py-2 items-center text-right">
              <div className="flex flex-col">
                ...
                <span className="text-xl font-semibold">{stats.proxies.num_total_reverse_proxies + stats.proxies.num_total_streams}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xl font-semibold">Reverse Proxies</span>
                <span className="text-xl font-semibold">{stats.proxies.num_total_reverse_proxies}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xl font-semibold">Streams</span>
                <span className="text-xl font-semibold">{stats.proxies.num_total_streams}</span>
              </div>
            </div>
          </CardBody>
        </Card>
        <Card className="xl:max-w-sm bg-default-50 rounded-xl shadow-md px-4">
          <CardBody className="py-5">
            <div className="flex flex-col">
              ...
              <span className="text-xs">{Object.keys(stats.proxies.providers).length} Providers</span>
            </div>
            <Spacer y={4}></Spacer>
            <div className="gap-2 grid grid-cols-2 items-center text-right">
              {
                Object.entries(stats.proxies.providers).map(([name, props], index) => (
                  <div className="flex gap-2 items-center" key={`provider_${name}`}>
                    <FontAwesomeIcon className="w-4" icon={props.type == 'docker' ? faDocker : faFile} color={props.type == 'docker' ? '#46ffff' : palette[index]} />
